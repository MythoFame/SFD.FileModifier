    ...
    try
        ...
        | Some secondOffset ->
            ...
            | Some index ->
                ...
                let newFileBytes =
                    Array.concat [ beforeHeader; headerBytes; replacementBytes; afterTerminator ]
        ...
        | Some(offset) ->
            ...
            | Some(mOffset) ->
                ...
                | Some(tIndex) ->
                    ...
                    let finalFileBytes =
                        Array.concat [ beforeTerminator; publishIdWithPrefix; afterTerminator ]
...
let modifyVersion (filePath: string, versionCode: string) =
    ...
    try
        ...
        | None -> printfn "Error: Start byte not found in the file."
    with
    | :? IOException as ex -> printfn "I/O error: %s" ex.Message
    | ex -> printfn "An error occurred: %s" ex.Message
...
let printHelp () =
    printfn "Usage: SFD.MapModifier.fsx -f <file> [options]"
    printfn ""
    printfn "Options:"
...
let printVersion () = printfn "%s" version
...
let runMode (filePath: string) (mode: string) (modeArg: string option) =
    ...
    | "4" ->
        match modeArg with
        | Some arg ->
            let isValidPublishId (id: string) =
                id |> String.forall Char.IsDigit && id.Length >= 10

            if isValidPublishId arg then
                modifyPublishIdHeader (filePath, arg)
            else
                printfn "Error: Publish ID must be at least 10 digits long and contain only numeric characters."
    ...
    | other -> printfn "Error: Unknown mode '%s'. Valid modes are 1–5." other
...
let rec parseArgs (tokens: string list) (acc: CliArgs) : CliArgs =
    match tokens with
    | [] -> acc
    | ("-h" | "--help") :: rest -> parseArgs rest { acc with ShowHelp = true }
    | ("-V" | "--version") :: rest -> parseArgs rest { acc with ShowVersion = true }
    | ("-i" | "--interactive") :: rest -> parseArgs rest { acc with Interactive = true }
    | ("-f" | "--file") :: value :: rest ->
        parseArgs
            rest
            { acc with
